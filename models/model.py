...
class StageNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, num_depth, cost_regularization, stage_idx=0, gt_depth=None, topic_init=None):
        ...
        for feat, src_proj, topic_init_per_pair in zip(features, src_projs, topic_init):
            ...
            #     ref_theta, src_theta = theta
            #     ref_theta = rearrange(ref_theta, "n (h w) c -> n c h w", h=ref_h)
            #     src_theta = rearrange(src_theta, "n (h w) c -> n c h w", h=src_h)
            #     warped_theta_vol = homo_warping_3D(src_theta, src_proj_new, ref_proj_new, depth_values)
            #     sim_vol = (ref_theta.unsqueeze(2).repeat(1, 1, num_depth, 1, 1) * warped_theta_vol).sum(dim=1)
            # else:
            #     sim_vol = in_prod_vol.mean(dim=1)
            #     sim_vol_norm = F.softmax(sim_vol.detach(), dim=1)
            #     entropy = (- sim_vol_norm * torch.log(sim_vol_norm)).sum(dim=1, keepdim=True)
            # entropy = (- topic_sim_vol * torch.log(topic_sim_vol + 1e-8)).sum(dim=1, keepdim=True)
...
class CDSMVSNet(nn.Module):
    def __init__(self, num_stages=3, depth_hypotheses=(48, 32, 8), depth_intervals_ratio=(4, 2, 1), share_cr=False,
        ...
        print("**********netphs:{}, depth_intervals_ratio:{},  grad:{}, chs:{}************".format(depth_hypotheses,
        ...
        for i in range(self.num_stages):
            ...
            if i == 0:
                ...
                snet = StageNet(self.feature.out_channels[i], nheads[i], topic_dim=None, **topicfm_cfg)
