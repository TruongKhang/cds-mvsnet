...
class TanksandTemplesDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=192, interval_scale=1.06, **kwargs):
        super(TanksandTemplesDataset, self).__init__()
    ...
    def read_depth(self, filename, new_h, new_w):
        # read pfm depth file
        depth = np.array(Image.open(filename), dtype=np.float32) / 1000
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            depth_filename = os.path.join(self.datapath, '{}/rendered_depth_maps/{:0>8}.png'.format(scan, vid))
            mask_filename = os.path.join(self.datapath, '{}/rendered_depth_maps/{:0>8}.png'.format(scan, vid))

            img = self.read_img(img_filename)
            intrinsics, extrinsics, depth_min, depth_interval = self.read_cam_file(proj_mat_filename)

            img, intrinsics, scaled_h, scaled_w = self.scale_img_cam(img, intrinsics, max_h=576)
